...
{
    return (uint8_t*)&(gdtTable->dataSegmentSelector) - (uint8_t*)gdtTable; //(uint8_t*) point to  the 1byte datatype
}

uint16_t codeSegmentSelector(GlobalDescriptorTable* gdtTable)
{
    return (uint8_t*)&(gdtTable->codeSegmentSelector) - (uint8_t*)gdtTable;
}

uint32_t Base(GlobalDescriptorTable* gdtTable)
{
    uint8_t* target = (uint8_t*)gdtTable;
