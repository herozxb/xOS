...
int handle_scrolling(int cursor_offset) {
	...
	for(i = 1; i < MAX_ROWS; i ++) {
		memory_copy((char*)(get_screen_offset(0, i) + get_terminal_buf_addr()),
					(char*)(get_screen_offset(0, i-1) + get_terminal_buf_addr()), MAX_COLS * 2);
	}
	/* Blank the last line by setting all bytes to 0 */
	char* last_line = (char*)(get_screen_offset(0, MAX_ROWS -1) + get_terminal_buf_addr());
...
void putchar(char character, int col, int row, char attribute_byte) {
	...
	unsigned char* vidmem = (unsigned char*) get_terminal_buf_addr();
